...
//
// Taksks with higher priority are unscheduled first. If two tasks have the same
...
// Schedule adds a new [Task] with the given value and priority to the queue.
...
// escalation policy to the queue.
...
func (s *Scheduler[T]) Escalate(task *Task[T], target Priority) {
	...
	// Task has already been dequeued, or does not exist in the queue.
	if task.index < 0 {
		return
	}
...
// UnscheduleTask unschedules the specified [Task] from the scheduler. This is
...
// Less determines if the task at i less than the task at j? If so, then the
// task at i has higher priority.
